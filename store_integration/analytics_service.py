...
store_analytics_service = StorePerformanceAnalyticsService()
    ...
    def _calculate_delivery_metrics(self, shop: Shop, start_date: datetime, end_date: datetime) -> Dict:
        """
        Calculate delivery performance metrics.
        """
        try:
            # Get store reviews with delivery ratings
            delivery_reviews = StoreReview.objects.filter(
                shop=shop,
                created_at__gte=start_date,
                created_at__lte=end_date,
            ...
            metrics = {
                'average_delivery_rating': 0.0,
                'delivery_reviews_count': delivery_reviews.count(),
                'estimated_delivery_days': shop.average_delivery_days,
                'delivery_performance_score': 0.0
            }
            
            if delivery_reviews.exists():
                avg_delivery_rating = delivery_reviews.aggregate(
                    avg_rating=Avg('delivery_rating')
                )['avg_rating']
                metrics['average_delivery_rating'] = float(avg_delivery_rating or 0)
            
            # Calculate delivery performance score
            delivery_score = 5.0
            
            # Base score on delivery rating
            if metrics['average_delivery_rating'] > 0:
                delivery_score = metrics['average_delivery_rating']
            else:
                # Fallback to estimated delivery time
                if shop.average_delivery_days <= 2:
                    delivery_score = 5.0
                elif shop.average_delivery_days <= 5:
                    delivery_score = 4.0
                elif shop.average_delivery_days <= 7:
                    delivery_score = 3.0
                elif shop.average_delivery_days <= 14:
                    delivery_score = 2.0
                else:
                    delivery_score = 1.0
            
            metrics['delivery_performance_score'] = delivery_score
